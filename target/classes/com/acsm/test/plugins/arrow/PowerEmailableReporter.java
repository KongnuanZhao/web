package com.acsm.test.plugins.arrow;
...
import com.acsm.test.plugins.arrow.utils.ConfigReader;
...
public class PowerEmailableReporter implements IReporter
{
    ...
    private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details)
    {
        if (tests.getAllResults().size() > 0)
        {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite))
            {
                ...
                // buff.append("<td><a href=\"#m" + methodId + "\">" +
                // qualifiedName(method) + " "
                // + (description != null && description.length() > 0 ? "(\"" +
                // description + "\")" : "") + "</a>"
                // + (null == testInstanceName ? "" : "<br>(" + testInstanceName
                // + ")") + "</td><td>"
                // + this.getAuthors(className, method) + "</td><td
                // class=\"numi\">" + resultSet.size() + "</td>"
                // + "<td>" + (count == 0 ? "" : count) + "</td>" + "<td>" +
                // parameterString + "</td>" + "<td>"
                // + start + "</td>" + "<td class=\"numi\">" + (end - start) +
                // "</td>" + "</tr>");
                ...
                buff.append("<td><a href=\"#m" + methodId + "\">" + qualifiedName(method) + " "
                        ...
                        + this.getAuthors(className, method) + "</td><td class=\"numi\">"
                        + this.getDescription(className, method) + "</td>" + "</td>" + "<td>" + df.format(start) + "</td>"
    ...
    private void startResultSummaryTable(String style)
    {
        ...
        // m_out.println("<tr><th>Class</th><th>Method</th><th>Authors</th><th>#of<br/>Scenarios</th><th>Running
        // Counts</th>" +
        // "<th>Parameters</th><th>Start</th><th>Time<br/>(ms)</th></tr>");
        m_out.println("<tr><th>模块名称</th><th>方法名称</th><th>作者</th>"
                + "<th>描述</th><th>开始</th><th>执行时间<br/>(ms)</th></tr>");
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites)
    {
        ...
        m_out.print("<tr>");
        tableColumnStart("用例名称");
        tableColumnStart("方法数");
        tableColumnStart("通过数");
        tableColumnStart("跳过的测试用例");
        tableColumnStart("错误的测试用例");
        tableColumnStart("耗时");
        tableColumnStart("已运行的组");
        tableColumnStart("未运行的组");
        m_out.println("</tr>");
     ...
     * @author hzjingcheng
     */
    private String getDescription(String className, ITestNGMethod method)
    {
        JavaClass cls = builder.getClassByName(className);
        ...
        {
            allDescription = "unknown";
        } else
        {
            for (DocletTag description : descriptions)
            {
                allDescription += description.getValue() + " ";
            }
        }
        // get method author name
        JavaMethod[] mtds = cls.getMethods();
