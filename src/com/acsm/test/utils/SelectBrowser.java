...
public class SelectBrowser {
	...
	public WebDriver selectExplorerByName(String browser, ITestContext context, String localhost) {
		...
		String chromedriver_server = PropertiesDataProvider.getTestData(driverConfgFilePath, "chromedriver_server");
		String firefox_win = PropertiesDataProvider.getTestData(driverConfgFilePath, "firefox_win");
		...
		if (currentPlatform.toLowerCase().contains("win")) { // 如果是windows平台
			...
			if (browser.equalsIgnoreCase("ie")) {
				...
				options.addArguments("disable-extensions=Chrome Automation Extension");
				// 返回谷歌浏览器对象
				return new ChromeDriver(options);
			} else if (browser.equalsIgnoreCase("firefox")) {
				System.setProperty("webdriver.firefox.bin", "E:\\firefox35\\firefox.exe");
				System.setProperty("webdriver.gecko.driver", firefox_win);
				FirefoxProfile profile = new FirefoxProfile();
				// 设置下载路径
				profile.setPreference("browser.download.folderList", 2);// 0是下载到默认位置，2下载到指定位置
				profile.setPreference("browser.download.manager.showWhenStarting", "True");// 是否显示开始，true为显示，false为不显示
				profile.setPreference("browser.download.dir", "D:\\hava");// 设置下载路径
				profile.setPreference("browser.helperApps.neverAsk.saveToDisk", "application/octet-stream");// 指定要下载页面的content-type值，"application..."为文件的类型

				// 返回火狐浏览器对象
				return new FirefoxDriver(profile);

			} else if (browser.equalsIgnoreCase("ch_remote")) {
				WebDriver remoteDriver = SeleniumUtil.getRemoteDriver(new Remote("chrome", localhost));
				// 返回ghost对象
				return remoteDriver;
